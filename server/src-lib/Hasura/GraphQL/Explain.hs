...
explainGQLQuery
  ...
  -> GQLExplain
  -> m EncJSON
explainGQLQuery isPgCtx sc sqlGenCtx enableAL (GQLExplain query userVarsRaw) = do
  ...
  case rootSelSet of
    GV.RQuery selSet ->
      runInTx txAccess $ encJFromJValue <$> traverse (explainField userInfo gCtx sqlGenCtx) (toList selSet)
    GV.RMutation _ ->
      throw400 InvalidParams "only queries can be explained"
    GV.RSubscription rootField -> do
      (plan, _) <- E.getSubsOp isPgCtx gCtx sqlGenCtx userInfo queryReusability rootField
